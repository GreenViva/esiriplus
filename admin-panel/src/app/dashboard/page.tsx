...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [
      ...
      // consultations that carry a fee (> 0).
      supabase
        .from("consultations")
    ...
    const queryErrors = [
      ...
      paymentsRes.error,
      recentDoctorsRes.error,
      recentConsultationsRes.error,
      logsRes.error,
    ].filter((e): e is NonNullable<typeof e> => e !== null && e !== undefined);

    setErrors(queryErrors.map((e) => e.message));

    const totalRevenue = (paidConsultationsRes.data ?? []).reduce(
      (sum, c) =>
        FAILED_STATUSES.has((c.status ?? "").toLowerCase())
          ? sum
    ...
    <div>
      ...
      {errors.length > 0 && (
        <div className="mb-4 p-4 rounded-xl bg-red-50 border border-red-200 flex items-start justify-between">
          ...
          <button
            onClick={() => { setErrors([]); setLoading(true); fetchData(); }}
